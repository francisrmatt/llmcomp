...
from llama.llms import completion_fns, nll_fns, tokenization_fns, context_lengths
...
def generate_predictions(
    ...
    **kwargs
    ...
    if parallel and len(input_strs) > 1:
        # This is is the issue because we have to return scores
        ...
        with ThreadPoolExecutor(min(max_concurrent, len(input_strs))) as p:
            ...
            return_list = list(tqdm(p.map(complete, input_strs), total=len(input_strs)))

            completions_list = []
            scores_list = []
            for i in return_list:
                completions_list.append(i[0])
                scores_list.append(i[1])

    else:
        #completions_list = [complete(input_str) for input_str in tqdm(input_strs)]
        completions_str, scores_list = complete(input_strs[0]) 
        completions_list = [completions_str]
    ...
    return preds, completions_list, input_strs, scores_list
...
def get_llmtime_predictions_data(train, test, model, settings, num_samples=10, temp=0.7, alpha=0.95, beta=0.3, basic=False, parallel=True, **kwargs):
    ...
    # Create our own scaler with no scaling
    # This might cause problems with values that are less than 100
    scalers = [Scaler(lambda x: x, lambda x: x) for i in range(len(train))]

    # transform input_arrs
    input_arrs = [train[i].values for i in range(len(train))]

    # Increase length to not mess with zip
    #input_arrs.append(input_arrs[0])
    ...
    #print('appending 1 to the front')
    #input_strs.append(input_strs[0] + '0')
    ...
    # We don't have to worry about truncation here, just take input_arrs as the input
    
    steps = test_len # How many 'true-future' steps to test
    samples = None
    medians = None
    completions_list = None
    if num_samples > 0:
        preds, completions_list, input_strs, scores = generate_predictions(completion_fn, input_strs, steps, settings, scalers,
